...
const getAllCombinations = () => {
  return (
    db
      .query(
        `SELECT combinations.id AS id, colours.value AS colour, combinations.user_id, combinations_users.user_id AS saved_by
      ...
      .then((result) => {
        ...
        for (const row of result.rows) {
          ...
          if (combinations[id]) {
            if (!combinations[id].colours.includes(colour)) combinations[id].colours.push(colour);
            if (!combinations[id].savedBy.includes(savedBy) && savedBy) combinations[id].savedBy.push(savedBy);
          } else {
            combinations[id] = {
              id,
              colours: [colour],
...
const showSavedCombinations = (user_id) => {
  return db
    .query(
      `SELECT combinations_users.combination_id, combinations_users.user_id AS user_id, colours.value AS colour
      FROM combinations_users
